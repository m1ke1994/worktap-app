...
const deadline = ref(3) // срок по умолчанию
...
// --- Навигация с определением направления ---
function nextStep() {
...
const canNext = computed(() => {
  ...
  if (currentStep.value === 2)
    return description.value && requirements.value && deadline.value
...
<template>
  <div class="bg-[#f7faff] min-h-screen flex flex-col items-center py-10">
    ...
    <Transition
      :name="prevDirection === 'forward' ? 'slider' : 'slider-back'"
      mode="out-in"
      appear
    >
      <div
        :key="currentStep"
        class="w-[933px] bg-white rounded-xl p-8 shadow border border-gray-100 transition-all duration-300 space-y-6"
      >
        <!-- Шаг 1: Основные -->
        <template v-if="currentStep === 1">
          <h2 class="font-bold text-xl mb-6 text-[#473d69]">Опишите ваш запрос</h2>
          <div class="mb-8 grid grid-cols-2 gap-8">
            <div>
              <label class="block text-gray-500 mb-1 font-semibold">Название задачи <span class="text-red-500">*</span></label>
              <input
                v-model="workTitle"
                type="text"
                placeholder="Например: Перевезти мебель из IKEA"
            ...
            <div class="grid grid-cols-2 gap-8">
              <div>
                ...
                <select v-model="selectedCategory" class="w-full border-b border-gray-200 py-2 bg-transparent focus:outline-none text-gray-700">
              ...
              <div>
                ...
                <select v-model="selectedSubcategory" :disabled="!selectedCategory" class="w-full border-b border-gray-200 py-2 bg-transparent focus:outline-none text-gray-700">
          ...
          <div class="text-xs text-gray-400 mt-2">
            <b>Подсказка:</b> Опишите задачу, выберите категорию — это поможет найти подходящего исполнителя.
          </div>
        </template>

        <!-- Шаг 2: Описание + Требования + Дедлайн -->
        <template v-if="currentStep === 2">
          <h2 class="font-bold text-xl mb-6 text-[#473d69]">Детальное описание задачи</h2>
          <div class="mb-8">
            <label class="block text-gray-600 mb-1 font-semibold">Описание задачи <span class="text-red-500">*</span></label>
            <textarea
              v-model="description"
              rows="7"
              placeholder="Подробно опишите задачу, что требуется сделать, нюансы, пожелания…"
            ...
            <div class="text-xs text-gray-400 mt-1">
              Чем подробнее вы опишете задачу, тем лучше результат!
            </div>
          </div>
          <div class="mb-8">
            <label class="block text-gray-600 mb-1 font-semibold">Требования к исполнителю <span class="text-red-500">*</span></label>
            <textarea
              v-model="requirements"
              rows="4"
              placeholder="Что требуется от исполнителя (например: опыт, навыки, наличие инструмента…)"
              class="w-full border border-gray-200 rounded-xl p-4 text-base bg-[#faf9fc] focus:outline-none focus:border-[#7d6dbe] transition"
            ></textarea>
          </div>
          <div>
        ...
        <template v-if="currentStep === 3">
          <h2 class="font-bold text-xl mb-6 text-[#473d69]">Добавьте иллюстрации или файлы к задаче</h2>
          <div class="mb-8">
            <label class="block text-gray-600 mb-2 font-semibold">Изображения (до 5)</label>
            ...
            <div class="text-xs text-gray-400 mt-2">
              Загрузите фото задачи, примеры, чертежи — это поможет исполнителю лучше понять вашу цель.
            </div>
          </div>
          <div class="mb-8">
            <label class="block text-gray-600 mb-2 font-semibold">Документы, примеры, архивы (опционально)</label>
        ...
        <template v-if="currentStep === 4">
          <div class="flex flex-col items-center">
            ...
            <p class="text-gray-600 mb-8 text-center max-w-[500px]">
              Всё готово — проверьте введённые данные.<br> После публикации ваша задача появится в поиске и станет доступна для исполнителей.
            </p>
          </div>
        </template>

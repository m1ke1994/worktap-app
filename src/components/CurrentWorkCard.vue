    ...
    <transition name="fade">
      ...
      >
        <div
          ...
          @click.stop
        >
          <div class="md:w-1/3 flex flex-col items-center justify-center bg-gradient-to-tr from-[#f7faff] to-[#e9e6fb] py-10 px-6 md:rounded-l-2xl border-r border-[#ede8fc] min-w-[230px]">
            <div
            ...
            <div class="text-center text-gray-400 text-xs mb-6">ID: {{ work.owner_id }}</div>
            <div class="text-[#1DBF73] font-bold text-2xl mb-1">{{ formatPrice(work.price) }}</div>
          ...
          <div class="flex-1 flex flex-col px-8 py-8 overflow-y-auto" style="max-height: 90vh;">
            <button
            ...
            <h2 class="font-bold text-2xl mb-3 break-words">{{ work.title }}</h2>
            <div class="mb-4">
...
.work-card {
  box-shadow: 0 2px 16px 0 rgba(87, 74, 183, 0.06), 0 1.5px 5px 0 rgba(87, 74, 183, 0.04);
  will-change: box-shadow;
}
.work-card:hover {
  box-shadow: 0 4px 24px 0 rgba(87, 74, 183, 0.10), 0 3px 12px 0 rgba(87, 74, 183, 0.07);
}

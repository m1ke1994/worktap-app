...
async function completeWork() {
  ...
  const res = await fetch(
    `http://localhost:8000/api/works/my/${props.work.id}/complete/`,
    {
      method: "POST",
...
<template>
  <transition name="fade">
    ...
    >
      <div
        class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl md:flex overflow-hidden relative"
      >
        <!-- Левая панель с владельцем и статусом -->
        <div
          class="md:w-1/3 flex flex-col items-center justify-center bg-gradient-to-tr from-[#f7faff] to-[#e9e6fb] p-8 md:rounded-l-2xl border-r border-[#ede8fc] min-w-[200px]"
        >
          <div
            class="w-20 h-20 rounded-full bg-gray-200 flex items-center justify-center text-3xl font-bold mb-3 shadow overflow-hidden"
          >
            <template v-if="props.work.owner_avatar">
              <img
                :src="props.work.owner_avatar"
                alt="Аватар пользователя"
                class="w-full h-full object-cover"
              />
            </template>
            <template v-else>
              {{ props.work.owner_first_name?.[0] ?? ""
              }}{{ props.work.owner_last_name?.[0] ?? "" }}
            </template>
          </div>
 ...
 <div class="text-gray-400 text-xs mb-1">
  ...
  >
    {{ getDaysLeft(props.work) }}
        ...
        <div
          class="flex-1 flex flex-col px-6 py-8 overflow-y-auto"
        ...
        >
          ...
          <div class="flex gap-2 text-xs mb-2 flex-wrap">
            ...
            <span
              v-if="props.work.subcategory"
              class="bg-[#e8fbf5] text-[#1DBF73] rounded px-2 py-1"
              >{{ props.work.subcategory }}</span
            >
          </div>
